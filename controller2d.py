...
"""
# to convert degree to radian, multiply by pi/180. radian to degree,mult by 180/pi
...
class Controller2D(object):
    ...
    def set_throttle(self, input_throttle):
        ...
        throttle           = np.fmax(np.fmin(input_throttle, 1.0), 0.0) #np.fmax is used to clamp the throttle to a value greater than 0 always. np.fmin is to keep it less than 1.
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            # I (integral of velocity error term)
            inte_v = self.vars.integral_error_previous + e_v * st 

            # D
            derivate = (e_v - self.vars.error_previous) / st

            acc = kp * e_v + ki * inte_v + kd * derivate    #total error term. obtained by adding all the individual error values.

            if acc > 0:
                throttle_output = (np.tanh(acc) + 1)/2  #the formula for calculating throttle angle
            ...
            k_e = 0.3 #constant in the stanley controller crosstrack error.
            slope = (waypoints[-1][1]-waypoints[0][1])/ (waypoints[-1][0]-waypoints[0][0]) #overall slope calculated between first waypoint and last waypoint
            a = -slope
            b = 1.0
            c = (slope*waypoints[0][0]) - waypoints[0][1]  #a,b,c are the constants for the reference path equation

            # heading error
            yaw_path = np.arctan2(waypoints[-1][1]-waypoints[0][1], waypoints[-1][0]-waypoints[0][0]) #tan-1(y/x) written as tan-1(y,x)
            ...
            crosstrack_error = np.min(np.sum((current_xy - np.array(waypoints)[:, :2])**2, axis=1)) #axis=1 means we're summing the rows
